...
def post_tweet(text: str, reply_to_id: str | None = None) -> str:
    """Post a single tweet via Composio v2 API. Returns tweet ID."""
    ...
    data = resp.json()
    log.info(f"Composio response: {json.dumps(data)[:300]}")

    if not data.get("successfull", False):
        raise RuntimeError(f"Tweet failed: {data.get('error', data.get('message', data))}")
